...
#Guardar la informacion de las muetras que se seleccionaron
...
#keep <- filterByExpr(d, min.count = 10, min.prop = 0.5, group=group) ## If a phenotype (group) would like to be considered in the filter step
keep <- filterByExpr(d)                               
#table(keep)
summary(keep)
...
RNA.norm_cpm<- cpm(d, normalized.sizes = TRUE, log = TRUE)
